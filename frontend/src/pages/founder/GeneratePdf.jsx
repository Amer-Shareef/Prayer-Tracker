...
const getPrayerStatistics = async (memberId, memberJoinDate = null) => {
  try {
    ...
    console.log(
      `📊 Fetching prayer statistics for member ${memberId} for past ${daysToCheck} days`
    );
...
const generateMemberReport = async (member) => {
  ...
  doc.text("Member Comprehensive Report", pageWidth / 2, 30, {
    align: "center",
  });
  ...
  const daysText = member.joined_date
    ? (() => {
        ...
        return daysSinceJoined < 40
          ? `Prayer Statistics - Past ${
              daysSinceJoined + 1
            } Days (Since Joining)`
          : "Prayer Statistics";
      })()
  ...
  try {
    ...
    const prayerStats = await getPrayerStatistics(
      member.id,
      member.joined_date
    );
    ...
    if (prayerStats.length === 0) {
      ...
      doc.setTextColor(secondaryColor);
      doc.text(
        "No prayer statistics available for this member.",
        margin,
        yPosition
      );
      yPosition += 10;
      ...
      while (currentDayIndex < prayerStats.length) {
        const dayChunk = prayerStats.slice(
          currentDayIndex,
    ...
    doc.text(
      "Error loading prayer statistics. Please try again.",
      margin,
      yPosition
    );
  }
  ...
  // Save the PDF
  const fileName = `Member_Report_${
    member.username || member.memberId || "Unknown"
