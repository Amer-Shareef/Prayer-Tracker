...
const AreaPage = () => {
    ...
    try {
      ...
      if (editingArea) {
        ...
        await areaService.updateArea(
          editingArea.area_id || editingArea.id,
          submissionData
        );
  ...
  const handleSubAreaSubmit = async (e) => {
    ...
    const token = localStorage.getItem("token");
    console.log(
      "🔍 Debug - Creating sub-area with token:",
    ...
    try {
      if (editingSubArea) {
        ...
        await areaService.updateSubArea(
          currentAreaId,
          editingSubArea.id,
          subAreaFormData.address
        );
        // Update state without full refresh
        setSubAreas((prev) => ({
          ...prev,
        ...
        // Create new sub-area
        const response = await areaService.createSubArea(
          currentAreaId,
          subAreaFormData.address
        );
        // Add to state without full refresh
        const newSubArea = response.data?.subArea || {
          id: Date.now(),
          address: subAreaFormData.address,
        };
        setSubAreas((prev) => ({
          ...prev,
  ...
  const handleDelete = async (area) => {
    if (window.confirm("Are you sure you want to delete this area?")) {
      try {
        ...
        setAreas((prevAreas) =>
          prevAreas.filter(
            (a) => (a.area_id || a.id) !== (area.area_id || area.id)
          )
        );
        ...
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <p className="text-gray-500 mt-2">
              This page is only accessible to Working Committee Admins and Super
              Administrators.
            </p>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Manage geographical areas and their associated founders and
              members
            </p>
          </div>
          <button
                    ...
                    <React.Fragment key={area.area_id || area.id}>
                      ...
                      {expandedAreas.has(area.area_id || area.id) && (
                        <tr>
                          <td
                            colSpan="4"
                            className="px-6 py-4 bg-gradient-to-r from-green-50 to-gray-50"
                          >
                            <div className="ml-8">
                                ...
                                <>
                                  ...
                                  0 ? (
                                    <div className="grid gap-3">
                                      {subAreas[area.area_id || area.id].map(
                                        (subArea) => (
                                          <div
                                            key={subArea.id}
                                    ...
                                    <div className="text-center py-8 bg-white rounded-lg border-2 border-dashed border-gray-300">
                                      ...
                                      <p className="text-xs text-gray-400 mt-1">
                                        Click "Add Sub Area" to create one
                                      </p>
                                    </div>
                                  )}
                                </>
                  ...
                  <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                    <div>
                      <p className="text-sm text-gray-700">
                        ...
                        of <span className="font-medium">{areas.length}</span>{" "}
                        areas
                        {totalPages > 1 && (
              ...
              <form onSubmit={handleSubAreaSubmit}>
                <div className="mb-4">
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setSubAreaFormData({ address: e.target.value })
                    }
