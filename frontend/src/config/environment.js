...
export const ENV_CONFIG = {
  ...
  HEALTH_CHECK_URL: process.env.REACT_APP_HEALTH_CHECK_URL || 'http://localhost:5000/api/health',
  ...
  // Allowed hosts
  ALLOWED_HOSTS: process.env.REACT_APP_ALLOWED_HOSTS?.split(',') || ['localhost', '127.0.0.1']
};
...
export const getFrontendUrl = (path = '') => {
  return `${ENV_CONFIG.FRONTEND_URL}${path.startsWith('/') ? path : `/${path}`}`;
};
